...
# Severity ordering: CRITICAL=1 … INFO=5 (lower = more severe)
SEVERITY_RANK = case(
...
async def _search_vulnerabilities(
    db_factory, tenant_id: uuid.UUID, query: str, limit: int
) -> list[dict[str, Any]]:
    """Search vulnerabilities by cve_id, vulnerability_name, affected_product, remediation_action."""
...
async def _search_assets(
    ...
    """Search assets by hostname, serial_number, ip_addresses (text), os_name."""
    async with db_factory() as db:
        pattern = f"%{query}%"
        stmt = (
            select(
...
async def _search_users(
    ...
    """Search users by email, display_name, department."""
    async with db_factory() as db:
        pattern = f"%{query}%"
        stmt = (
            select(
...
async def _search_tickets(
    ...
    """Search tickets by cve_id (via vulnerability join) or external_ticket_url."""
    async with db_factory() as db:
        pattern = f"%{query}%"
        ...
        stmt = (
            ...
            .where(
                ...
                or_(
                    vuln.cve_id.ilike(pattern),
                    Ticket.external_ticket_url.ilike(pattern),
                ),
            )
...
async def _search_cspm(
    db_factory, tenant_id: uuid.UUID, query: str, limit: int
) -> list[dict[str, Any]]:
    """Search CSPM findings by rule_id, rule_name, resource_name, resource_id."""
    async with db_factory() as db:
        pattern = f"%{query}%"
        stmt = (
            select(
...
async def global_search(
    ...
    log = logger.bind(query=q, limit=limit, tenant_id=str(user.tenant_id))
    log.info("global_search.start")

    # Run all five searches in parallel, each with its own DB session
    vuln_task = _search_vulnerabilities(async_session_factory, user.tenant_id, q, limit)
    asset_task = _search_assets(async_session_factory, user.tenant_id, q, limit)
    user_task = _search_users(async_session_factory, user.tenant_id, q, limit)
    ticket_task = _search_tickets(async_session_factory, user.tenant_id, q, limit)
    cspm_task = _search_cspm(async_session_factory, user.tenant_id, q, limit)

    vulns, assets, users, tickets, cspm = await asyncio.gather(
        vuln_task, asset_task, user_task, ticket_task, cspm_task,
        return_exceptions=True,
    )

    # Handle any exceptions — return empty list for failed categories
    ...
    for name, value in [
        ...
        if isinstance(value, BaseException):
            log.error("global_search.category_error", category=name, error=str(value))
            results[name] = []
        else:
            results[name] = value

    total = sum(len(v) for v in results.values())
    log.info("global_search.done", total=total)
