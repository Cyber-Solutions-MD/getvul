...
async def export_vulnerabilities_csv(db: AsyncSession, tenant_id: uuid.UUID, filters: dict) -> str:
    ...
    query = (
        ...
        .order_by(
            case((Vulnerability.severity == "CRITICAL", 0), (Vulnerability.severity == "HIGH", 1),
                 (Vulnerability.severity == "MEDIUM", 2), else_=3),
    ...
    writer.writerow(["CVE ID", "Severity", "Status", "Source", "Hostname", "Product", "Version",
                      "Fixed Version", "Exploit Available", "CISA KEV", "CVSS", "Remediation",
                      "File Paths", "First Detected", "Last Seen"])

    for vuln, hostname in rows:
        paths = ", ".join(vuln.file_paths) if vuln.file_paths and isinstance(vuln.file_paths, list) else ""
        writer.writerow([
            vuln.cve_id, vuln.severity, vuln.status, vuln.source, hostname,
            vuln.affected_product, vuln.affected_version, vuln.fixed_version,
            vuln.exploit_available, vuln.cisa_kev, vuln.cvss_v3_score,
            vuln.remediation_action or vuln.remediation_info, paths,
...
async def export_assets_csv(db: AsyncSession, tenant_id: uuid.UUID) -> str:
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Hostname", "Device Category", "OS", "OS Version", "Risk Score",
                      "Serial Number", "Model", "Assigned User", "Department",
                      "Host Status", "Last Seen", "Scanners"])

    for a in assets:
        scanners = ", ".join(a.seen_by_sources) if isinstance(a.seen_by_sources, list) else ""
        writer.writerow([
            a.hostname, a.device_category, a.os_name, a.os_version, a.risk_score,
            a.serial_number, a.model, a.assigned_user, a.department,
            a.host_status, a.last_seen_at.isoformat() if a.last_seen_at else "",
            scanners,
        ])
...
async def export_users_csv(db: AsyncSession, tenant_id: uuid.UUID) -> str:
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Email", "Display Name", "Role", "Department", "Job Title",
                      "Active", "Password Login", "IdP Source", "Groups", "Last Login"])

    for u in users:
        groups = ", ".join(u.groups) if u.groups and isinstance(u.groups, list) else ""
        writer.writerow([
            u.email, u.display_name, u.role, u.department, u.job_title,
            u.is_active, u.allow_password_login, u.idp_source, groups,
...
async def export_tickets_csv(db: AsyncSession, tenant_id: uuid.UUID) -> str:
    ...
    for ticket, cve_id, severity, hostname in rows:
        ...
        if url not in tasks:
            tasks[url] = {
                "url": url, "provider": ticket.provider, "status": ticket.external_status,
                "assignee": ticket.assignee, "hostname": hostname, "created": ticket.ticket_created_at,
                "resolved": ticket.resolved_at, "vulns": 0, "cves": [], "max_severity": "LOW",
            }
        tasks[url]["vulns"] += 1
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Ticket URL", "Provider", "Host", "Severity", "Vulns", "CVEs",
                      "Assignee", "Status", "Created", "Resolved"])

    for t in tasks.values():
        writer.writerow([
            t["url"], t["provider"], t["hostname"], t["max_severity"], t["vulns"],
            "; ".join(t["cves"][:10]), t["assignee"], t["status"],
...
async def export_remediations_csv(db: AsyncSession, tenant_id: uuid.UUID) -> str:
    ...
    query = (
        select(
            ...
            func.max(case(
                (Vulnerability.severity == "CRITICAL", 4), (Vulnerability.severity == "HIGH", 3),
                (Vulnerability.severity == "MEDIUM", 2), else_=1,
            )).label("max_sev"),
        )
        .where(
    ...
    for r in rows:
        writer.writerow([r.remediation_id, r.remediation_action, r.affected_product,
                          sev_map.get(r.max_sev, "LOW"), r.hosts, r.vulns])
...
async def _collect_summary_data(db: AsyncSession, tenant_id: uuid.UUID, filters: dict | None = None) -> dict:
    ...
    critical = (await db.execute(select(func.count(Vulnerability.id)).where(*open_filter, Vulnerability.severity == "CRITICAL"))).scalar_one() if not sev_filter or "CRITICAL" in sev_filter else 0
    high = (await db.execute(select(func.count(Vulnerability.id)).where(*open_filter, Vulnerability.severity == "HIGH"))).scalar_one() if not sev_filter or "HIGH" in sev_filter else 0
    exploitable = (await db.execute(select(func.count(Vulnerability.id)).where(*open_filter, Vulnerability.exploit_available.is_(True)))).scalar_one()
    kev_count = (await db.execute(select(func.count(Vulnerability.id)).where(*open_filter, Vulnerability.cisa_kev.is_(True)))).scalar_one()
    ...
    cat_q = select(Asset.device_category, func.count(), func.avg(Asset.risk_score)).where(*asset_where).group_by(Asset.device_category)
    ...
    # Tickets
    ticket_total = (await db.execute(select(func.count(func.distinct(Ticket.external_ticket_url))).where(Ticket.tenant_id == tenant_id))).scalar_one()
    ticket_open = (await db.execute(select(func.count(func.distinct(Ticket.external_ticket_url))).where(
        Ticket.tenant_id == tenant_id, Ticket.resolved_at.is_(None)))).scalar_one()

    return {
        "org": tenant.name, "generated": now,
        "sections": sections,
        "vulns": {"total": total, "open": total, "critical": critical, "high": high, "exploitable": exploitable, "kev": kev_count},
        "assets": {"total": total_assets, "by_category": by_category, "risk": {"critical": rd[0], "high": rd[1], "medium": rd[2], "low": rd[3]}},
        "top_hosts": [{"hostname": h.hostname, "risk": h.risk_score, "type": h.device_category, "user": h.assigned_user} for h in top_hosts],
        "top_remediations": [{"action": r.remediation_action, "product": r.affected_product, "hosts": r.hosts, "vulns": r.vulns, "severity": sev_map.get(r.max_sev, "LOW")} for r in top_rems],
...
async def generate_executive_summary(db: AsyncSession, tenant_id: uuid.UUID, filters: dict | None = None) -> str:
    ...
    if "vulns" in sec:
        lines.extend(["", "VULNERABILITY OVERVIEW",
            f"  Total (filtered):           {v['total']:>8,}", f"  Critical:                   {v['critical']:>8,}",
            f"  High:                       {v['high']:>8,}", f"  Exploitable:                {v['exploitable']:>8,}",
            f"  CISA KEV:                   {v['kev']:>8,}"])
    if "assets" in sec:
    ...
    if "risk" in sec:
        lines.extend(["", "RISK DISTRIBUTION",
            f"  Critical (80+):             {a['risk']['critical']:>8,}", f"  High (50-79):               {a['risk']['high']:>8,}",
            f"  Medium (20-49):             {a['risk']['medium']:>8,}", f"  Low (<20):                  {a['risk']['low']:>8,}"])
    if "top_hosts" in sec:
        lines.extend(["", f"TOP {n} RISKIEST HOSTS",
            f"  {'Hostname':<30s} {'Risk':>5s}  {'Type':<15s} {'User':<20s}", f"  {'-'*75}"])
        for h in d["top_hosts"]:
            lines.append(f"  {h['hostname']:<30s} {h['risk']:>5d}  {(h['type'] or '-'):<15s} {(h['user'] or '-'):<20s}")
    if "top_remediations" in sec:
        lines.extend(["", f"TOP {n} REMEDIATIONS",
            f"  {'Product':<25s} {'Sev':<10s} {'Hosts':>6s} {'Vulns':>6s}  Action", f"  {'-'*80}"])
        for r in d["top_remediations"]:
            lines.append(f"  {(r['product'] or '?'):<25s} {r['severity']:<10s} {r['hosts']:>6,} {r['vulns']:>6,}  {(r['action'] or '')[:50]}")
    if "tickets" in sec:
        lines.extend(["", "TICKET STATUS",
            f"  Total:                      {t['total']:>8,}", f"  Open:                       {t['open']:>8,}",
...
async def generate_executive_summary_csv(db: AsyncSession, tenant_id: uuid.UUID, filters: dict | None = None) -> str:
    ...
    for label, val in [("Total", v["total"]), ("Open", v["open"]), ("Critical", v["critical"]),
                        ("High", v["high"]), ("Exploitable", v["exploitable"]), ("CISA KEV", v["kev"])]:
    ...
    for label, val in [("Critical (80+)", a["risk"]["critical"]), ("High (50-79)", a["risk"]["high"]),
                        ("Medium (20-49)", a["risk"]["medium"]), ("Low (<20)", a["risk"]["low"])]:
        w.writerow([label, val])
    w.writerow([])
...
async def generate_executive_summary_pdf(db: AsyncSession, tenant_id: uuid.UUID, filters: dict | None = None) -> bytes:
    ...
    pdf.cell(0, 6, f"Organization: {d['org']}  |  Generated: {d['generated'].strftime('%Y-%m-%d %H:%M UTC')}", new_x="LMARGIN", new_y="NEXT")
    ...
    for label, val in [("Total Vulnerabilities", f"{v['total']:,}"), ("Open / In Progress", f"{v['open']:,}"),
                        ("Critical (open)", f"{v['critical']:,}"), ("High (open)", f"{v['high']:,}"),
                        ("Exploitable (open)", f"{v['exploitable']:,}"), ("CISA KEV (open)", f"{v['kev']:,}")]:
        row(label, val)
    pdf.ln(5)
    ...
    pdf.cell(60, 6, "  Category"); pdf.cell(30, 6, "Count", align="R"); pdf.cell(40, 6, "Avg Risk", align="R", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 9)
    for cat, count, avg in sorted(a["by_category"], key=lambda x: -x[1]):
        pdf.cell(60, 5, f"  {cat}"); pdf.cell(30, 5, f"{count:,}", align="R"); pdf.cell(40, 5, f"{avg:.1f}", align="R", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "B", 9)
    pdf.cell(60, 5, "  TOTAL"); pdf.cell(30, 5, f"{a['total']:,}", align="R", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    ...
    pdf.set_font("Helvetica", "", 9)
    for h in d["top_hosts"]:
        pdf.cell(55, 5, f"  {h['hostname'][:25]}"); pdf.cell(20, 5, str(h["risk"]), align="R")
    ...
    for r in d["top_remediations"]:
        pdf.cell(45, 5, f"  {(r['product'] or '?')[:20]}"); pdf.cell(20, 5, r["severity"])
        pdf.cell(15, 5, str(r["hosts"]), align="R"); pdf.cell(15, 5, str(r["vulns"]), align="R")
