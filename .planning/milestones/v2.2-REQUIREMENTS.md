...
- [ ] **UX-D-03-02**: All text + UI meets WCAG 2.1 AA contrast (4.5:1 text, 3:1 UI/graphics) in light mode on every route.
- [ ] **UX-D-03-03**: Severity / status / SLA pills and severity glyphs are legible and mutually distinct on light surfaces.
...
- [ ] **UX-D-03-05**: The e2e a11y sweep (`e2e/a11y-routes.spec.ts`) runs under `data-theme="light"` as well as dark, and is green.

## Phase 17 — Page-transition motion (from UX-D-06)

- [ ] **UX-D-06-01**: Route changes within the `(authed)` shell animate with a cross-fade/transition via the **View Transitions API** (single `template.tsx`).
...
- [ ] **UX-D-06-03**: A CSS-animation fallback keeps navigation clean in browsers without View Transitions support (no jank/broken nav in Firefox).
- [ ] **UX-D-06-04**: Transitions do not race with DrillPanel Esc/clickaway close and cause no layout shift.
...
- [ ] **UX-D-01-01**: The board view renders four status columns (Open / In progress / Completed / Blocked) populated from `useTickets`, replacing the "coming soon" placeholder.
- [ ] **UX-D-01-02**: A ticket can be moved between columns by pointer drag (**@dnd-kit**), persisting the new status via a mutation with optimistic update + rollback on error.
