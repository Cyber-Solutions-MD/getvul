...
1. **Does "Last quarter" (D-03) mean a rolling ~90-day window or an actual calendar quarter?**
   ...
   - Recommendation: Implement "Last quarter" as the most-recently-**completed** calendar quarter (a simple, well-defined date-arithmetic function — sketch given in Code Examples). This matches how a board actually uses the phrase ("Q2's numbers") and avoids a confusing duplicate-feeling preset next to "Last 90 days."
...
4. **Should SOC 2 ship 1 control (CC7.1 only) or 2 (CC7.1 + CC7.2)?**
   ...
   - Recommendation: Ship CC7.1 alone if catalog credibility is the priority; add CC7.2 only if the team is comfortable with its broader framing. Flagged explicitly rather than silently picking one.
