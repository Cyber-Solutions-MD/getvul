...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 11-01-01 | 01 backend | 0 | UX-03-02 (sort), UX-03-05 (group) | — | `require_viewer` enforced; severity allow-list rejects bad input | pytest | `pytest backend/tests/test_vuln_sort.py -x` | ❌ W0 | ✅ green |
| 11-01-02 | 01 backend | 0 | UX-03-01 (facets), D-F-02 | T-IDOR | Facet counts respect tenant scoping; `require_viewer` enforced | pytest | `pytest backend/tests/test_vuln_facets.py -x` | ❌ W0 | ✅ green |
| 11-01-03 | 01 backend | 0 | UX-03-05, D-V-01 | T-IDOR | `?group=host` respects tenant scope; pagination math correct | pytest | `pytest backend/tests/test_vuln_group_host.py -x` | ❌ W0 | ✅ green |
| 11-01-04 | 01 backend | 0 | UX-03-03 (action: create ticket), D-P-04 | T-CSRF, T-AuthZ | `POST /tickets` requires `require_analyst`; 401 surfaces clean | pytest | `pytest backend/tests/test_tickets_create.py -x` | ⚠️ verify exists | ✅ green |
| 11-02-01 | 02 hooks | 1 | UX-03-04, D-F-05 | T-XSS | `useUrlStateList` XSS-clamps via allow-list before any render | unit | `npx vitest run hooks/use-url-state-list.test.ts` | ❌ W0 | ✅ green |
| 11-02-02 | 02 hooks | 1 | UX-S-03, D-S-03 | — | `useQueryErrors` re-renders only on error transitions (snapshot fingerprint) | unit | `npx vitest run lib/queries/use-query-errors.test.tsx` | ❌ W0 | ✅ green |
| 11-02-03 | 02 hooks | 1 | UX-03-01..03, D-F-02 | — | `useVulnerabilities` keys include filters; 401 retry restricted (Phase 10 BL-06) | unit | `npx vitest run lib/queries/use-vulnerabilities.test.tsx` | ❌ W0 | ✅ green |
| 11-02-04 | 02 hooks | 1 | UX-03-03 action, D-P-04 | T-CSRF, T-AuthN | `useCreateTicketMutation` throws `Session expired during mutation` on 401 (no retry) | unit | `npx vitest run lib/mutations/use-create-ticket.test.tsx` | ❌ W0 | ✅ green |
| 11-03-01 | 03 states | 1 | UX-S-01, D-S-01 | — | `SkeletonTable` honors `prefers-reduced-motion`; forced-colors safe | unit + axe | `npx vitest run components/states/skeleton-table.test.tsx` | ❌ W0 | ✅ green |
| 11-03-02 | 03 states | 1 | UX-S-02, D-S-02 | — | `EmptyState` compound API; `role="status"`; axe-clean | unit + axe | `npx vitest run components/states/empty-state.test.tsx` | ❌ W0 | ✅ green |
| 11-03-03 | 03 states | 1 | UX-S-03, D-S-03 | — | `PartialFailureBanner` hybrid mode; `role="alert"`; axe-clean; HTTP+request-id surfaced | unit + axe | `npx vitest run components/states/partial-failure-banner.test.tsx` | ❌ W0 | ✅ green |
| 11-03-04 | 03 states | 1 | D-V-02 | — | `PerSourceStatusStrip` `aria-live="polite"`; composes connectors + facets | unit + axe | `npx vitest run components/states/per-source-status-strip.test.tsx` | ❌ W0 | ✅ green |
| 11-04-01 | 04 vuln components | 2 | UX-03-01, D-F-01/02/03/05 | T-XSS | `ChipBar` debounce search 250ms; chips sync URL immediately; XSS clamp on display | unit | `npx vitest run components/vulnerabilities/chip-bar.test.tsx` | ❌ W0 | ✅ green |
| 11-04-02 | 04 vuln components | 2 | UX-03-02, UX-07-03, D-T-01/02/04, D-V-04 | — | 7 columns + 4 sortable; keyboard (Enter/Space/↑↓/Home/End/Esc); stale-row tint | unit | `npx vitest run components/vulnerabilities/vuln-table.test.tsx` | ❌ W0 | ✅ green |
| 11-04-03 | 04 vuln components | 2 | UX-03-03, D-P-01/02/05/06 | — | Drill panel desktop: close × / Esc / outside-click / row-swap; focus on close button | unit | `npx vitest run components/vulnerabilities/drill-panel.test.tsx` | ❌ W0 | ✅ green |
| 11-04-04 | 04 vuln components | 2 | UX-03-06, D-P-03 | — | Mobile drill via vaul bottom-sheet at <900px; matchMedia mock | unit | `npx vitest run components/vulnerabilities/drill-panel-mobile.test.tsx` | ❌ W0 | ✅ green |
| 11-04-05 | 04 vuln components | 2 | UX-03-05, D-V-01 | — | `ViewToggle` swaps mode without losing filter state | unit | `npx vitest run components/vulnerabilities/view-toggle.test.tsx` | ❌ W0 | ✅ green |
| 11-05-01 | 05 page rewrite | 2 | UX-03-01..06, UX-S-01..04 | — | Page renders chip-bar + table + panel; URL ↔ state round-trip; loading/empty/partial-failure paths | integration | `npx vitest run app/.../vulnerabilities/page.test.tsx` | ❌ W0 | ✅ green |
| 11-06-01..06 | 06 phase-10 retrofit | 3 | D-S-06, UX-S-01..03 | — | Each retrofit site renders canonical primitive; dashboard visual continuity preserved | unit | per-component test reruns | per-site | ✅ green |
| 11-07-01 | 07 dev primitives | 3 | UX-S-01..03 (showcase) | — | `/dev/primitives` renders showcase in dev + 404s in prod (BL-05 DCE gate) | unit | `cd frontend && npx vitest run src/app/dev/primitives/page.test.tsx` | ✅ | ✅ green |
| 11-08-01 | 08 a11y + UAT | 3 | UX-07-03 (slice) | — | Manual keyboard + screen-reader walkthrough on `/vulnerabilities` | manual | HUMAN-UAT checklist (`11-HUMAN-UAT.md`) | — | ⬜ manual |
...
Reconciled against the shipped test suite. One genuine automated gap was found and closed;
one manual walkthrough is escalated to manual-only.
...
| Covered (green) | 19 |
| Gaps found | 1 |
| New tests written | 1 (`src/app/dev/primitives/page.test.tsx` — D-31/BL-05 dev-render + prod-404 gate) |
| Escalated to manual-only | 1 (`11-08-01` keyboard + SR walkthrough → `11-HUMAN-UAT.md`) |

Row `11-07-01` previously pointed at a never-written `npx vitest run app/dev/primitives`; it now
maps to the new route-gate test (2/2 green). Evidence: full vitest suite green + backend
sort/facets/group/create test files present. **Nyquist-compliant** (all automated closed; one
documented manual item).
