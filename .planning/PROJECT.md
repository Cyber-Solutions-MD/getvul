...
---
*Last updated: 2026-06-01 — Phase 12 (`/assets` list + detail) complete: 8/8 plans across 4 waves landed clean. Backend: tags ARRAY column + GIN index (alembic 025), aggregated sla_breach count, os_family allow-list filter, POST /assets/{id}/owner reassign route, GET /tickets?asset_id= filter. Frontend: 3 new primitives (RiskRing SVG with sunset gradient, Breadcrumb, Avatar with 2-char initials per sketch), 1 utility (osFamily), generic descriptor-driven ChipBar primitive replacing Phase 11's vuln-specific one (vuln page migrated to thin wrapper), 5 TanStack hooks (useAssets/useAssetDetail/useAssetVulnerabilities/useAssetRemediations/useAssignableUsers + asset_id thread through VulnerabilitiesFilters), full /assets list page rewrite (386→state-primitive-composed), two-column /assets/[id] detail page with sticky 340px right rail (RiskCard + OwnerCard with inline Reassign combobox) and main column (severity ribbon + vulns-on-this-host with DrillPanel reuse + remediation timeline). Tests: 426/426 frontend green across 71 files (+5 over Phase 11). Code review: 18 findings (4 blocker + 14 warning) all fixed inline + 1 missed slice caught in iter-2 + 4 info-level polish items deferred. HUMAN-UAT (5 items: responsive sticky rail, DrillPanel URL contract, reassign optimistic flow, malformed-input UX, cross-tenant 404) pending manual walkthrough.*
