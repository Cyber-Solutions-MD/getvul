...
---
*Last updated: 2026-05-27 — Phase 11 (`/vulnerabilities` + state patterns) complete: backend faceted-counts / by-host grouping / expanded sort axes (Literal-typed Query params, HTTP 400 on unknown), 4 canonical cross-phase state primitives (SkeletonTable, EmptyState, PartialFailureBanner, PerSourceStatusStrip — locked APIs for Phases 12-14), 7 vuln-page components (chip-bar, view-toggle, vuln-table, drill-panel desktop + vaul mobile, drill-content, microcopy), live `/dashboard/vulnerabilities` page rewrite (658→250 lines), Pagination restyled to sunset tokens, 5 Phase 10 dashboard sites retrofitted onto canonical primitives, dev primitives showcase extended (BL-05 dead-code-elimination preserved — 124 B prod chunk). Bundle: `/dashboard/vulnerabilities` 22.3 kB / 155 kB First-Load JS. Tests: 291/291 green across 49 frontend files. HUMAN-UAT (8 items: mobile vaul gesture, screen readers, forced-colors, reduce-motion, sketch fidelity, 2k-vuln perf) pending manual walkthrough.*
