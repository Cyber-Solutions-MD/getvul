...
echo "🔧 Fixing seed.py — use savepoints for vuln inserts..."
...
echo "🔄 Resetting database and restarting..."
...
echo "🔍 Testing stats..."
curl -s "http://localhost:8000/api/v1/vulnerabilities/stats" -H "Authorization: Bearer dev-token" | python3 -m json.tool 2>/dev/null || curl -s "http://localhost:8000/api/v1/vulnerabilities/stats" -H "Authorization: Bearer dev-token"
echo ""

echo "🔍 Testing vuln list..."
curl -s "http://localhost:8000/api/v1/vulnerabilities?page_size=3" -H "Authorization: Bearer dev-token" | python3 -m json.tool 2>/dev/null || echo "Failed"
echo ""

echo ""
echo "✅ Now check:"
echo "   http://localhost:3000/dashboard"
echo "   http://localhost:3000/dashboard/vulnerabilities"
echo ""
echo "📝 Then commit:"
echo "   git add -A && git commit -m 'fix: use savepoints in seed to prevent full rollback' && git push"
