...
echo "🔧 Fixing remediation drill-down filters..."

# ══════════════════════════════════════════════
#  Backend: Add filter params to drill-down endpoints
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
#  Backend: Update router to pass filters to drill-downs
# ══════════════════════════════════════════════

# Rewrite the remediation endpoints in the router
python3 << 'PYEOF'
content = open("backend/app/vulnerabilities/router.py").read()

# Replace the remediation routes block
old_block = '''# ── Remediation views ──
...
open("backend/app/vulnerabilities/router.py", "w").write(content)
print("Updated router with filter params")
PYEOF

# ══════════════════════════════════════════════
#  Frontend: Pass filters to drill-down API calls
# ══════════════════════════════════════════════

python3 << 'PYEOF'
content = open("frontend/src/app/dashboard/vulnerabilities/page.tsx").read()

# Fix drillRemediation to pass filters
old_drill_rem = '''  async function drillRemediation(rem: RemediationGrouped) {
...
new_drill_rem = '''  async function drillRemediation(rem: RemediationGrouped) {
    setSelectedRemediation(rem);
    try {
      ...
      const hosts = await api<HostForRemediation[]>(`/api/v1/vulnerabilities/remediations/${encodeURIComponent(rem.remediation_id)}/hosts?${p}`);
      setRemHosts(hosts);
    } catch (e) { console.error(e); }
  }'''

content = content.replace(old_drill_rem, new_drill_rem)

# Fix drillHost to pass filters
old_drill_host = '''  async function drillHost(assetId: string, hostname: string) {
...
new_drill_host = '''  async function drillHost(assetId: string, hostname: string) {
    setSelectedHostId(assetId); setSelectedHostName(hostname);
    try {
      const p = new URLSearchParams();
      filters.severity.forEach((s) => p.append("severity", s));
      if (filters.exploit_available === true) p.set("exploit_only", "true");
      if (filters.cisa_kev === true) p.set("kev_only", "true");
      const rems = await api<RemediationForHost[]>(`/api/v1/vulnerabilities/hosts/${assetId}/remediations?${p}`);
      setHostRemediations(rems);
    } catch (e) { console.error(e); }
  }'''

content = content.replace(old_drill_host, new_drill_host)

open("frontend/src/app/dashboard/vulnerabilities/page.tsx", "w").write(content)
print("Updated frontend drill-downs with filters")
PYEOF
...
echo "✅ Fixed! Filters now apply to:"
echo "   - Remediations grouped list"
echo "   - Hosts for a specific remediation"
echo "   - Remediations for a specific host"
echo ""
echo "   Try: select CRITICAL filter → click a remediation → only CRITICAL vulns shown"
echo ""
echo "📝 Commit:"
echo "   git add -A && git commit -m 'fix: pass severity/exploit/KEV filters to remediation drill-downs' && git push"
